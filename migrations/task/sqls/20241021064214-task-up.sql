...
FROM "CREDIT_PURCHASE" WHERE user_id = (SELECT ID FROM "USER" WHERE email = 'wXlTq@hexschooltest.io')
GROUP BY user_id) AS "CREDIT_PURCHASE"
LEFT JOIN(SELECT user_id, COUNT(course_id) AS used_credit
FROM "COURSE_BOOKING" WHERE user_id = (SELECT ID FROM "USER" WHERE email = 'wXlTq@hexschooltest.io')
AND status = 'confirmed' GROUP BY user_id) AS "COURSE_BOOKING"
